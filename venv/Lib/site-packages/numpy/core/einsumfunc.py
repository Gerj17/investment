...
def _optimal_path(input_sets, output_set, idx_dict, memory_limit):
    ...
    on ``memory_limit`` and returns the lowest t_cost path. This algorithm
...
def _parse_possible_contraction(positions, input_sets, output_set, idx_dict, memory_limit, path_cost, naive_cost):
    """Compute the t_cost (removed size + flops) and resultant indices for
    ...
    path_cost : int
        The contraction t_cost so far.
    naive_cost : int
        The t_cost of the unoptimized expression.

    Returns
    -------
    t_cost : (int, int)
        A tuple containing the size of any indices removed, and the flop t_cost.
    positions : tuple of int
        The locations of the proposed tensors to contract.
    ...
    # Sieve based on total t_cost as well
...
def _greedy_path(input_sets, output_set, idx_dict, memory_limit):
    ...
    ``(-prod(indices_removed), t_cost)``.  What this amounts to is prioritizing
...
def einsum_path(*operands, **kwargs):
    ...
    Evaluates the lowest t_cost contraction order for an einsum expression by
...
def einsum(*operands, **kwargs):
    ...
    can greatly increase the computational efficiency at the t_cost of a larger
